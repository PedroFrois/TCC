...
A manutenibilidade de um sistema é uma das principais características que influenciam na qualidade de um \textit{software} \cite{arquiMonografiaKecia}. A partir dessa informação, foram levantados três tópicos como principais pontos para facilitar a manutenção de um projeto de \textit{software} \cite{arquiEngSoftModerna}. São eles:
...
Logo, esse princípio pode ser utilizado em diferentes níveis do \textit{software}: a nível de classes e funções; a nível de componente; e, o mais importante para o estudo em questão, a nível arquitetural do \textit{software} relacionando responsabilidades e os limites de cada camada da arquitetura. Além disso, o princípio condiz diretamente com a coesão de um módulo, ou seja, se seguido, a coesão do módulo será maior \cite{arquiEngSoftModerna}.
...
base * altura = area\end{math} para calcular sua área, assim demostrado no Código \ref{code:quadrilatero}.
...
Contudo, o \textit{software} que siga ao máximo o princípio da Inversão de Dependência será mais estável do que aquele que não o utiliza \cite{arquiCleanArch}.
