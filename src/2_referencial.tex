...
Na figura \ref{fig:androidStack} observa-se o \textit{Software Stack} do Android, ou a pilha de software, que seria a arquitetura genérica de um \textit{smartphone} que utiliza esse SO. Nela, é possível observar que a base de desenvolvimento do SO é um kernel Linux \cite{triviaTanenbaum2015modern}, que também se trata de um software de código aberto.

\begin{figure}[h]
    \caption{Android \textit{Software Stack}}
    \centering
    \includegraphics[width=.4\textwidth]{img/android stack info image.png}
...
%===================================================================================================================
\subsection{Swift}
Swift é a linguagem de programação de propósito geral criada e recomendada pela Apple para o desenvolvimento de \textit{softwares} para todo o ecossistema da própria empresa, \cite{iosStartDevIos,iosSwift}. Assim como o Kotlin, realiza teste de nulidade para evitar \textit{crashs} inesperados no software. 

Segundo a Apple, suas características são as seguintes \cite{iosSwift}:
...
Outro ponto relevante a ser mencionado sobre essa linguagem é a possibilidade de uso da biblioteca SwiftUi que possibilita o desenvolvimento de interfaces de usuário de forma declarativa. Isso faz com que o desenvolvimento seja mais produtivo, somado à compatibilidade nativa dessa interface com todos dispositivos Apple \cite{iosSwiftUI}.
...
A manutenibilidade de um sistema é uma das principais características que influenciam na qualidade de um software \cite{arquiMonografiaKecia}. A partir dessa informação, foram levantados três tópicos como principais pontos para facilitar a manutenção de um projeto de software \cite{arquiEngSoftModerna}. São eles:

\begin{enumerate}
  \item Ocultamento de Informação: estabelece que operações dinâmicas de um módulo não devem ser conhecidas pelo cliente. Isso faz com que alterações nessas partes dinâmicas não gerem modificações nos clientes desse módulo.
  
  \item Coesão: assim como a própria definição da palavra, é o grau de harmonia que as operações de um módulo possuem entre si \cite{triviaDicio}. Dessa forma, o módulo deveria ser responsável somente por funcionalidades que gerem somente \textbf{um} motivo\footnote{motivo no sentido negocial da regra, excluindo correções de \textit{bugs} e alterações técnicas} para alterações nele mesmo \cite{arquiEngSoftModerna}.
...
Em suma, para construir um software de boa qualidade é necessário ocultar as informações dinâmicas de um módulo; que o módulo seja coeso internamente; e que tenha o mínimo de acoplamento com outros módulos. Para atingir esses objetivos, foram criados os princípios de projetos, que traduzem em diretrizes mais concretas que os desenvolvedores conseguem e devem seguir. Entre os vários princípios de projetos, serão apresentados 05 (cinco), conhecidos como padrão SOLID \cite{arquiCleanArch}.

%===================================================================================================================
\subsection{\textit{Single Responsibility} - Princípio da Responsabilidade Única}
A definição do princípio da responsabilidade única é que um módulo deve ter um, e somente um, motivo para alteração. Em outras palavras o módulo deve atender a um único cliente/\textit{stakeholder} ou "grupo de interesse" \cite{arquiCleanArch}. 

Logo, esse princípio pode ser utilizado em diferentes níveis do software: a nível de classes e funções; a nível de componente; e, o mais importante para o estudo em questão, a nível arquitetural do software relacionando responsabilidades e os limites de cada camada da arquitetura. Além disso, o princípio condiz diretamente com a coesão de um módulo, ou seja, se seguido, a coesão do módulo será maior \cite{arquiEngSoftModerna}.

%===================================================================================================================
\subsection{\textit{Open-Closed} - Princípio Aberto-Fechado}
O princípio Aberto/Fechado defende que um módulo deve ser aberto para ser estendido e adaptado à novas funcionalidades, contudo, não deverá sofrer alterações internas para se adaptar à elas \cite{arquiCleanArch}.

Para que isso ocorra é necessário que o desenvolvedor separe o módulo em componentes de maneira a possibilitar a extensão de sua solução para o acréscimo de novas funcionalidades sem acessá-lo e, ainda, ocultar informações desnecessárias à sua extensibilidade.

%===================================================================================================================
\subsection{\textit{Liskov Substitution} - Princípio da Substituição de Liskov}
A Substituição de Liskov determina que classes que estendam métodos não podem quebrar os contratos dos métodos da classe estendida. Para facilitar o entendimento desse princípio, segue um exemplo de quebra:
...
base * altura = area\end{math} para calcular sua área, assim demonstrado no Código \ref{code:quadrilatero}.
...
Esse cálculo não está incorreto para o contexto de um quadrado. Entretanto, ao se utilizar uma instância da classe Quadrado como um Quadrilatero, o cálculo da área poderia apresentar erros, consoante representado no Código \ref{code:testeQuadrado}, uma vez que a área esperada é 10, mas irá resultar em 25.
...
Logo, não seguir o princípio da Substituição de Liskov pode gerar graves problemas, além de aumentar o custo de manutenção do sistema \cite{arquiCleanArch}. 

%===================================================================================================================
\subsection{\textit{Interface Segregation} - Princípio da Segregação de Interface}

A Segregação de Interface é um caso específico do princípio da Responsabilidade Única. Nesse novo contexto, utiliza-se de interfaces para separar funções das quais as classes precisam, a fim de que uma classe dependa de todas as operações de uma outra através das interfaces. 

Exemplificando: dado que uma classe A, que dependa da classe B, utilize um método b1, contudo, B possui os métodos b1 (utilizado por A) e b2 (não utilizado por A). O princípio da Segregação de Interface determina que deverá ser criada uma interface X que possua somente o método b1 para que A dependa de X, que, por sua vez, não possui métodos não utilizados por A \cite{arquiCleanArch}.

%===================================================================================================================
\subsection{\textit{Dependency Inversion} - Princípio da Inversão de Dependência}

A ideia base do princípio da Inversão da Dependência consiste na ideia de que interfaces são mais estáveis do que suas respectivas implementações. Com isso, para seguir o princípio é preferível que nenhuma classe dependa diretamente da implementação concreta de outra classe, mas sim de uma interface. A preferência é colocada no lugar da obrigatoriedade uma vez que não é possível que uma classe seja totalmente livre da dependência de implementações concretas. 

Contudo, o software que siga ao máximo o princípio da Inversão de Dependência será mais estável do que aquele que não o utiliza \cite{arquiCleanArch}.
